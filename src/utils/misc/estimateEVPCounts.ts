const getEVPCounts = (cards, setPrefix, evpCSRTotalPopulation) => {
  const getAllNonFoilCardsInSetByRarity = (rarity) => {
    ...
    return {
      ...
      population: editions.reduce((n, edition) => n + ((
        [...(edition.circulationTemplates ?? []), ...(edition.circulations ?? [])]
      ).find(circulation => circulation.foil !== true)?.population ?? 0), 0),
    }
  }
  ...
  const getTotalBoosterPackCount = () => (
