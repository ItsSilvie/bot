...
const generateCardsByMiscTemplate = async () => {
    ...
    const generateThingRender = (arr, thingKey, thingTitle, formatThing, config = {}) => {
        ...
        for (let i = 0; i < sortedThings.length; i++) {
            ...
            const { cards, editions, countFoil, countNonFoil, thing, } = sortedThings[i];
  ...
  <tbody>
    ...
    const generateCardByThing = (thingKey, thingTitle, formatThing, config = {}) => {
    ...
    const generateCardByThingArray = (thingKey, thingTitle, formatThing, config = {}) => {
        let reducedCards = cards;
        if (config.isCardKey && config.reducerFn) {
            reducedCards = cards.reduce((cardsArr, card) => {
                if (typeof config.reducerFn !== 'function') {
    ...
    generateCardByThing('element', 'Element', (element) => (`<div style="align-items: center; display: flex; column-gap: 8px;">
  <img src="https://img.silvie.org/misc/elements/${element.toLowerCase()}.png" width="24px" />
  <a href="https://index.gatcg.com/cards?element=${element}">
    ...
    generateCardByThing('rarity', 'Rarity', (rarity) => (`<a href="https://index.gatcg.com/cards?rarity=${rarity}">
  ${rarity_1.Rarity[(0, rarity_1.getRarityCodeFromRarityId)(rarity)]} (${(0, rarity_1.getRarityCodeFromRarityId)(rarity)})
</a>`), {
        isCardKey: false,
    });
    generateCardByThingArray('types', 'Type', (type) => (`<div style="align-items: center; display: flex; column-gap: 8px;">
    <img src="https://img.silvie.org/misc/types/${type.toLowerCase()}.png" width="24px" />
  <a href="https://index.gatcg.com/cards?type=${type}">
    ...
    generateCardByThingArray('classes', 'Class', (classEntry) => (`<a href="https://index.gatcg.com/cards?class=${classEntry}">
  ${classEntry}
</a>`), {
        isCardKey: true,
    });
    generateCardByThing('name', 'Card name', (name) => (`<a href="https://index.gatcg.com/cards?name=${name}">
  ${name}
</a>`), {
        isCardKey: true,
        limit: 10,
    });
    generateCardByThing('customName', 'Name', (name) => (`<a href="https://index.gatcg.com/cards?name=${name}">
  ${name}
</a>`), {
        reducerFn: (arr, card) => {
            if (!/, /.test(card.name)) {
