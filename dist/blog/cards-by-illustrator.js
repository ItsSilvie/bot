...
const generateCardsByIllustratorTemplate = async () => {
    ...
    for (let i = 0; i < sortedIllustrators.length; i++) {
        const { cards, editions, illustrator, countFoil, countNonFoil, } = sortedIllustrators[i];
  ...
  <tbody>
    ...
    for (let i = 0; i < sortedIllustratorsByPopulation.length; i++) {
        const { illustrator, countFoil, countNonFoil, } = sortedIllustratorsByPopulation[i];
...
<tbody>
  ${templateDataPopulation.join('\n')}
    ...
    for (let i = 0; i < Object.values(rarity_1.Rarity).length; i++) {
        const rarityId = i + 1;
        const rarityCode = (0, rarity_1.getRarityCodeFromRarityId)(i + 1);
        ...
        for (let i = 0; i < sortedIllustratorsByRarity.length; i++) {
            const { illustrator, rarities, } = sortedIllustratorsByRarity[i];
  ...
  <tbody>
    ...
    for (let i = 0; i < sortedIllustratorsPRCPR.length; i++) {
        const { illustrator, matchingRarityCount, matchingRarityEditions, matchingRarityPopulation, matchingRarityPopulationFoil, } = sortedIllustratorsPRCPR[i];
  ...
  <tbody>
    ...
    for (let i = 0; i < sortedIllustratorsCR.length; i++) {
        const { illustrator, matchingRarityCount, matchingRarityEditions, matchingRarityPopulation, matchingRarityPopulationFoil, } = sortedIllustratorsCR[i];
